...
const NavBar = () => {
	...
	return (
		...
		>
			...
			{/* TODO: animation border + animation np to nepftune */}
				<Link href={"/"} className='flex justify-start pt-10 pl-6'>
					<p className='text-3xl text-brown border-b-[1px] border-brown'>N</p>
					<p className={`${isOpen ? "hidden duration-150" : "block duration-150"} text-3xl text-brown border-b-[1px] border-brown`}>p.</p>
					<p className={`${isOpen ? "block duration-150" : "hidden duration-150"} text-3xl text-brown border-b-[1px] border-brown`}>epftune</p> 
				</Link>
